...
export function generatePhrasePassword(config: PhraseConfig): string {
  ...
  for (let i = 0; i < wordCount; i++) {
    // Selección por índices completamente aleatorios
    const wordIndex = Math.floor(Math.random() * dictLength);
  ...
  if (extraSuffix) {
    ...
    const randomDigit = Math.floor(Math.random() * 8) + 2; // de 2 a 9 (evita 0,1)
